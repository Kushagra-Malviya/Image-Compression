...
def plot_iterations(iterations, target=None):
    ...
    for i, img in enumerate(iterations):
        ...
        if target is None:
            ...
            # Ensure target and img have the same shape
            if target.shape != img.shape:
                target_resized = ndimage.zoom(target, (img.shape[0]/target.shape[0], img.shape[1]/target.shape[1]), order=0)
            else:
                target_resized = target
            # Display the RMSE
            plt.title(str(i) + ' (' + '{0:.2f}'.format(np.sqrt(np.mean(np.square(target_resized - img)))) + ')')
        frame = plt.gca()
...
def test_greyscale():
    ...
    mpimg.imsave('image_file_greyscale.jpg', image_array, cmap='gray')
...
def test_rgb():
    ...
    mpimg.imsave('image_file_rgb.jpg', image_array)
    plt.subplot(122)
